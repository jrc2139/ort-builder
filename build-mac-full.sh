#!/usr/bin/env bash
# Full build of ONNX Runtime for macOS (with all ops including Tokenizer/contrib)
...
# Pass a config file as argument to enable reduced ops: ./build-mac-full.sh model.config
ONNX_CONFIG=""
...
mv "${OUTPUT_LIB}" libs/macos-${ARCH}/

echo ""
echo "Static library created: libs/macos-${ARCH}/libonnxruntime_all.a"
ls -lh "libs/macos-${ARCH}/libonnxruntime_all.a"
...
echo "  cp libs/macos-${ARCH}/libonnxruntime_all.a /path/to/fastembed-zig/deps/onnxruntime-static/lib/"
echo ""
echo "Then build with: zig build -Dstatic=true"
